...
class GetScheduledAppByByPackageName @Inject constructor(
    private val repository: ScheduledAppRepository,
) {
    suspend operator fun invoke(packageName: String): ScheduledApp? {
        return repository.getByPackageName(packageName)
    }
}